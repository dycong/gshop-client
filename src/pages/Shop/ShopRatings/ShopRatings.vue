<template>
  ...
  <!--
    dispatch（getRatings）获取评价
    mapState获取info，ratings数据，将数据显示, 遍历ratings，定义日期过滤器。滑动BScroll
    使用全局组件：Star
    注册全局组件：Split  RatingsFilter
    过滤显示评价：根据selectType确定评价类型，根据onlyText确定评价是否有内容
                  条件1                      条件2
                 selectType: 0/1/2          onlyContent: true/false
                 rateType: 0/1              text: length>0 / length===0
   点击选择评价类型，点击切换显示是否有内容评价
...
<script>
  ...
  export default {
    data() {
      return {
        selectType: 2, // 0:好评  1：差评  2：全部
        onlyText: false  //是否只显示带文本评价
      }
    },
